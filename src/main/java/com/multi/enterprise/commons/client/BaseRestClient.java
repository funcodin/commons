...
import com.multi.enterprise.commons.exception.ClientException;
...
public class BaseRestClient {
	...
	public <T> T postForObject( final URI uri, final Object requestObject, final Class<T> responseType ) throws ServiceException, ClientException{
		try{
	...
	public <T> T putForObject(final URI uri, final Object requestObject, final Class<T> responseType) throws ServiceException, ClientException{
		try{
			final ResponseEntity<T> response = this.restTemplate.exchange(uri, HttpMethod.PUT, new HttpEntity<Object>(requestObject), responseType);
	...
	private void handleException( final HttpMethod httpMethod, final URI uri, final RestClientException restClientException ) throws ServiceException, ClientException{
		if(restClientException.getCause() != null && restClientException.getCause() instanceof HttpResponseException){
			...
			final ServiceException serviceException = ServiceException.valueOf(httpException.getStatusCode(), httpException.getMessage());
			if(serviceException != null ){
				throw serviceException;
			}
			throw new ServiceException("Status code "+ httpException.getStatusCode() +"Http method "+ httpMethod +" URL "+ uri +" Exception "+ httpException.toString());
		}
		throw new ClientException("Http method "+ httpMethod +" URL "+ uri +" Exception "+ restClientException.toString());
	}
}
